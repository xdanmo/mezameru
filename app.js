// ==========================================
// DROPBOX API SYNC LOGIC (PKCE OAuth)
// ==========================================

const CLIENT_ID = '695wnemrrpi6mrc';
let refreshToken = localStorage.getItem('mezameru_dropbox_refresh');
...
function logoutUser() {
    localStorage.removeItem('mezameru_dropbox_refresh');
    refreshToken = null;
    dbx = null;
    
    const btnLogin = document.getElementById('btn-dropbox-login');
...
async function syncWithDropbox() {
    ...
    try {
        const response = await dbx.filesDownload({ path: '/' + FILE_NAME });
        ...
        const errSum = error.error?.error_summary || "";
        
        // If file doesn't exist, we upload it
        if (errSum.includes("not_found") || error.status === 409) {
             await uploadToDropbox();
             finishSync();
        } 
        // If our token is invalid/revoked, force logout
        else if (error.status === 401 || errSum.includes("invalid_access_token") || errSum.includes("expired_access_token")) {
             logoutUser();
             reRenderAll();
        } 
        else {
...
async function uploadToDropbox() {
    ...
    try {
        await dbx.filesUpload({
            path: '/' + FILE_NAME,
            contents: JSON.stringify(APP_STATE),
            mode: 'overwrite'
        });
    } catch (error) { 
...
function saveState() {
    pruneOldData(); 
    saveLocal();
    if (refreshToken) uploadToDropbox();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DROPBOX PKCE AUTH INITIALIZATION ---
    const btnLogin = document.getElementById('btn-dropbox-login');
    if (btnLogin) {
    ...
    if (authCode) {
        ...
        dbxAuth.getAccessTokenFromCode(REDIRECT_URI, authCode)
            .then(response => {
                refreshToken = response.result.refresh_token;
                localStorage.setItem('mezameru_dropbox_refresh', refreshToken);
